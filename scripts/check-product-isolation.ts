...
function checkStaticBoundaries(repoRoot: string): void {
  ...
  assert(runnerBuilder.includes('appId: com.findmikeymike.runner'), 'Runner app ID changed unexpectedly');
  assert(runnerBuilder.includes('productName: Runner'), 'Runner product name changed unexpectedly');
