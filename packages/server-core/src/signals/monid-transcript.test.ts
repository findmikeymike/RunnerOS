...
import liveTools from './__fixtures__/monid-tools.json';
...
function fixture() {
  ...
  const client: PoolClient = {
    listTools: async () => liveTools as unknown as Awaited<ReturnType<PoolClient['listTools']>>,
    callTool: async (name, args) => {
      calls.push(name);
      if (name === 'monid_inspect') return inspection;
      if (name === 'monid_run') { expect(args).toEqual({ provider: 'apify', endpoint, input: { body: { youtube_url: `https://www.youtube.com/watch?v=${id}`, language: 'en' } } }); return run(); }
...
test('lower user cap and unhealthy inspection prevent paid work', async () => {
  ...
  await expect(monidSignalTranscript(f.root, id, undefined, f.deps)).rejects.toThrow();
  expect(f.calls).toEqual(['monid_inspect', 'monid_inspect']);
});
...
test('new scope authorizes fresh paid work only after terminal known-cost settlement', async () => {
  ...
  await expect(run('workflow-1')).rejects.toThrow();
  expect(f.calls.at(-1)).toBe('monid_get_run');
...
test('new scope cannot reset unknown charge or pending work', async () => {
  for (const state of ['FAILED', 'RUNNING', 'lost-ack']) {
    ...
    expect(f.calls.filter(name => name === 'monid_run')).toHaveLength(1);
...
test('preflight and completed paid failures block provider switching', async () => {
  ...
  expect(capped.calls).toEqual(['monid_inspect']);
  ...
  expect(unhealthy.calls).toEqual(['monid_inspect']);
  ...
  expect(failed.calls).toEqual(['monid_inspect', 'monid_run']);
  expect(failed.budget.getStatus().spentLast7DaysUsd).toBe(0.01);
});

test('preflight failures distinguish tool contracts from connection and allowance failures', async () => {
  const incompatible = fixture();
  const client = await incompatible.deps.createClient();
  client.listTools = async () => [];
  await expect(monidSignalTranscript(incompatible.root, id, undefined, incompatible.deps)).rejects.toThrow('collection tools are incompatible');
  expect(incompatible.calls).toEqual([]);
  const capped = fixture(); capped.budget.updateLimits(0.001, 1);
  await expect(monidSignalTranscript(capped.root, id, undefined, capped.deps)).rejects.toThrow('allowance');
  expect(capped.calls).toEqual(['monid_inspect']);
});

test('completed unusable output reports a data failure, not an allowance or login failure', async () => {
  const f = fixture(); f.setRun(() => ({ ...f.completed, output: undefined }));
  await expect(monidSignalTranscript(f.root, id, undefined, f.deps)).rejects.toThrow('returned data that Signals could not verify');
  expect(f.calls).toEqual(['monid_inspect', 'monid_run']);
});
