...
for (const change of ['cancel', 'replace'] as const) {
  test(`late recovered workflow after ${change} cancels only the obsolete admission`, async () => {
    const root = makeRoot()
    writeWork(root, [buildOrder({ status: 'running',
      execution: { type: 'workflow-run', workflowSlug: 'fixture', workflowDigest: 'v1', triggerInputs: {} },
      runs: [{ id: 'original-attempt', jobId: 'order-1', status: 'running', startedAt: '2026-07-10T14:00:00Z' }],
    })])
    ...
    const runner = new ScheduledWorkRunner({
      canRunBackgroundWork: () => true, withLock: createLock(),
      executeAgentTask: async () => { throw new Error('Must not dispatch') },
      startWorkflow: async () => { throw new Error('Must not dispatch') },
      recoverWorkflow: async () => { invoked = true; await released.promise; return { runId: 'original-workflow' } },
      abortWorkflowRun: async (ws, id) => { expect(ws).toBe(workspaceId); aborted.push(id) },
      readWorkflowRun: () => null, listOutputManifests: () => [],
    })
    const scan = runner.scanWorkspace(workspaceId, root, new Date('2026-07-10T14:01:00Z'))
    await waitFor(() => invoked)
    const current = readWork(root).items[0]!
    writeWork(root, [change === 'cancel' ? { ...current, status: 'canceled' } : {
      ...current, runs: [...current.runs, { id: 'replacement-attempt', jobId: current.id, status: 'running', startedAt: '2026-07-10T14:02:00Z', workflowRunId: 'replacement-workflow' }],
    }])
    released.resolve()
    const result = await scan
    expect(aborted).toEqual(['original-workflow'])
    expect(result.started).toBe(0)
    expect(result.failed).toBe(0)
    const saved = readWork(root).items[0]!
    expect(saved.status).toBe(change === 'cancel' ? 'canceled' : 'running')
