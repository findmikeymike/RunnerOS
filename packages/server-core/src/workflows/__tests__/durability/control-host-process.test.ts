...
for (const scenario of ['pending', 'approved', 'cancel'] as const) {
  const approveFirst = scenario === 'approved', late = false, cancel = scenario === 'cancel';
  test(`control service commands survive process deaths ${cancel ? 'through cancellation' : approveFirst ? 'with approval supersession' : 'through pause and resume'}`, async () => {
    ...
    const run = (stage: string) => startProcess([join(import.meta.dir, 'control-host-worker.ts'), root, endpoint, stage], env, process.execPath, 25000);
    try {
      if (!late) {
      ...
      const paused = run('pause-stop');
      try { await paused.line(line => line.startsWith('{"barrier":"control-saved"')); }
      finally { paused.child.kill('SIGKILL'); await paused.done; }
      ...
      const control = run(cancel ? 'cancel-stop' : 'resume-command-stop');
      try {
        const saved = JSON.parse(await control.line(line => line.startsWith('{"barrier":"control-saved"')));
        expect(saved.result.state.status).toBe(cancel ? 'cancelled' : 'running');
      } finally { control.child.kill('SIGKILL'); await control.done; }
      if (cancel) {
        const stopped = await run('resume').done;
        expect(stopped.code).toBe(0);
        expect(stopped.stdout).toContain('"result":"cancelled"');
