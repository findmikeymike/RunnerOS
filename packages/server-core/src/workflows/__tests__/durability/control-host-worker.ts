/** Workflow control service with actual host/factory/Pi recovery, only in supervisor-owned synthetic configuration. */
...
const controls = new DurableWorkflowControls({ journal, runner: {
  decide: async command => ({ receipt: journal.decide(command) }),
  control: async command => ({ receipt: journal.command(command) }),
  steer: async command => ({ receipt: journal.steer(command) }),
}, resolvePrincipal: (_workspace, actor) => {
  if (actor.clientId !== 'fixture-client') throw new Error('unknown-control-actor');
...
try {
  if (stage === 'start' || stage === 'start-late-boundary') {
  ...
  } else if (['pause-stop', 'resume-command-stop', 'cancel-stop'].includes(stage)) {
    const action = stage === 'pause-stop' ? 'pause' : stage === 'cancel-stop' ? 'cancel' : 'resume';
    const result = await controls.control(input.workspaceId, input.runId, { commandId: stage, expectedVersion: journal.get(input.runId, input.workspaceId).version, action }, actor);
    console.log(JSON.stringify({ barrier: 'control-saved', result }));
    await new Promise(() => setInterval(() => {}, 1000));
    ...
    for (const [index, text] of ['Focus on radio first.', 'Only local stations.'].entries()) {
      ...
      const request = { commandId: command.commandId, expectedVersion: command.expectedVersion, action: command.action, text: command.text };
      const receipt = (await controls.control(input.workspaceId, input.runId, request, actor)).receipt;
      assert.deepEqual((await controls.control(input.workspaceId, input.runId, request, actor)).receipt, receipt);
