...
for (const changedPolicy of [false, true]) {
  test(`actual host approval survives process deaths ${changedPolicy ? 'and revalidates changed policy' : 'without repeating model work'}`, async () => {
    ...
    const run = (stage: string) => startProcess([join(import.meta.dir, 'approval-host-worker.ts'), root, endpoint, stage], env, process.execPath, 25000);
    try {
