...
test(lifecycle ? 'real Pi executes through host lifecycle and reopen reuses completion' : 'real host default factory authenticates Pi IPC, reads a native file, and commits completion', async () => {
  ...
  const child = spawn(process.execPath, [join(import.meta.dir, 'durable-read-default-factory.fixture.ts')], {
    cwd: root, env: { ...process.env, DURABLE_HOST_FIXTURE_ROOT: root, DURABLE_HOST_LIFECYCLE: lifecycle ? '1' : '0', CRAFT_CONFIG_DIR: join(root, 'config'),
