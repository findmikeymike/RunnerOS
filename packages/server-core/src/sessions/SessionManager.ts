    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const rawVideoEditorRepurposeWorkflowUpdated = replaceBuiltInAgentPromptText(
            'raw-video-editor',
            '10. Self-check `render-report.json` or the master-sync report, cut boundaries, captions, audio pops, aspect ratio, and duration before presenting the result.',
          ...
          if (rawVideoEditorDirectionSkillUpdated || rawVideoEditorDirectionPromptUpdated || rawVideoEditorPromptUpdated || rawVideoEditorForceGuidanceUpdated || rawVideoEditorRepurposePromptUpdated || rawVideoEditorRepurposeWorkflowUpdated || rawVideoEditorMetadataUpdated) {
            sessionLog.info('[agent-definitions] Updated existing Raw Video Editor direction, repurposing, and song-master synchronization')
          }
