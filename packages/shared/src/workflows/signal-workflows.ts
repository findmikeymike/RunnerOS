...
const reportContract = [
  ...
  'Keep at most 12 findings and 5 ideas. Finding shape: {id,title,excerpt,topics,sourceRefs,temporalKind,eventDate?}. Idea shape: {id,title,excerpt,topics,sourceRefs,temporalKind,eventDate?,supportingFindingIds,suggestedWorkerRoles?}. IDs are unique stable short strings. Each excerpt is your concise summary (at most 600 characters) for saved insights. It need not repeat the report verbatim and is not a proof quotation. Topics: at most 8 short tags. sourceRefs must be exact host packet source IDs, never invented URLs or IDs. Each idea cites existing supporting finding IDs and uses their sources. Zero ideas is valid.',
...
/** Installed definitions upgrade only through the exact-stock migration in seedSignalWorkflows. */
export function createSignalContractWorkflow(track: SignalTrack, mode: SignalMode): { slug: string; metadata: WorkflowMetadata; body: string } {
  ...
  return {
    slug,
    metadata: {
      name, description: name + ' with one direct transcript-to-intelligence pass.', avatar: 'SI',
      ...
      steps: [
        {
          ...
          description: 'Read the transcripts once and return the most useful intelligence for this artist.',
          input: [header, 'Read the supplied plain transcripts directly. Pick the strongest useful insights; skip repetition, filler, weak ideas, and generic advice. Do not request timestamps, proof excerpts, extra research, or another analysis pass. For Your World, explain useful discoveries about the artist interests, themes and wider world before offering optional stories, tensions, questions or creative angles. Do not manufacture controversy or force a release tie-in. For Industry, focus on actual music-business implications and recommend at most three supported actions. Prefer approved artist text over inferred interests; missing artist context is not a research blocker.', reportContract, packet].join('\n\n'),
          outputSchema: structuredClone(SIGNAL_SYNTHESIS_OUTPUT_SCHEMA), timeout: 300, onFailure: 'stop',
          completion: { requireNonEmptyOutput: true, minOutputChars: 1, maxAgentMessages: 0 },
        },
      ],
      outputs: { mode: 'none' },
    },
    body: '# ' + name + '\n\nSignals v3 host-admitted execution only. Read supplied transcripts once; return useful intelligence directly. No automatic context fanout or audio. Explicit setup adoption is required; this definition does not replace weekly-signal-scan.\n',
  };
}
