...
export function normalizeWorkflowTriggerInputs(
  ...
  // These are reserved runner controls rather than workflow business inputs.
  // Preserve them for the runner's defensive parser while continuing to drop
  // every other undeclared field.
  if (Object.prototype.hasOwnProperty.call(raw, 'enabled_source_slugs')) {
    out.enabled_source_slugs = raw.enabled_source_slugs;
  }
  if (Object.prototype.hasOwnProperty.call(raw, 'permission_mode')) {
    out.permission_mode = raw.permission_mode;
  }
  return out;
}
