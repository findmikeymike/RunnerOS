...
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      if (slug === 'field-theory') {
      ...
      } else if (slug === 'trypost') {
        sources.push({ ...getTryPostSource(workspaceId, workspaceRootPath), tier: 'project' });
      } else if (slug === 'postiz') {
        sources.push({ ...getPostizSource(workspaceId, workspaceRootPath), tier: 'project' });
      ...
      } else if (slug === 'lyrics-transcriber') {
        sources.push({ ...getLyricsTranscriberSource(workspaceId, workspaceRootPath), tier: 'project' });
        ...
        sources.push({ ...getAdsOperatorSource(workspaceId, workspaceRootPath), tier: 'project' });
