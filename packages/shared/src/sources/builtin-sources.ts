  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '4. If campaign folders are involved, list assets with `node src/social.mjs assets --asset-root <dir> --platform <platform> --json` and text with `node src/social.mjs content --content-root <dir> --json`.',
        '5. Default engine is `runner-cdp`: save dry-run JSON as the action contract, then run `node src/social.mjs execute --action-file <dry-run-result.json> --expected-action-id <act_...> --confirm yes --json` after explicit approval.',
        '6. Treat `browserPlan.accountVerification` as mandatory before submit; `social execute` rejects handoffs without a known expected handle or account URL.',
        '7. Dry-run posts, comments, and DMs with `--asset-root`, `--content-root`, relative filenames, and `--dry-run --json` before live execution.',
        '8. Use the returned `RUNNER_CDP_DELEGATED` browser plan with Runner native browser tools. If the visible account and draft match the approved dry-run, submit without asking again; stop only on mismatch, ambiguity, unexpected platform choices, or upload/UI failure.',
        '',
