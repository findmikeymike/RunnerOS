...
export function getGoogleAdsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const config: FolderSourceConfig = {
    ...
    tagline: 'Bundled Google Ads API v24.2-aware CLI for account discovery, GAQL reporting, diagnostics, and approval-gated operations.',
    icon: 'G',
    isAuthenticated,
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '',
        'Bundled build: `2026.6.25-runneros-v24`, targeting Google Ads API `v24` with v24.2 release coverage. Google Ads API v24.2 was released on 2026-06-24; the REST endpoint path remains `v24`.',
        '',
        'Workflow:',
        '1. Use the displayed local path as the working directory.',
        ...
        '6. For any proposed Google Ads write, run a `--dry-run` preview first and show the exact object, operation payload, reason, risk, and expected impact.',
        '7. Ask for explicit approval before any live mutation to campaigns, budgets, keywords, audiences, conversions, billing, or status.',
        '',
