...
export function getGenesisLyricSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const config: FolderSourceConfig = {
    ...
    tagline: 'Bundled single-video lyric clip engine from Genesis: audio, lyrics, image refs, captions, and FFmpeg assembly.',
    icon: '🎙️',
    isAuthenticated: true,
    connectionStatus: existsSync(toolPath) ? 'connected' : 'failed',
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '',
        'Core commands:',
        '1. `node bin/genesis-lyric.mjs doctor --json` to verify FFmpeg and the vendored Genesis engine.',
        '2. `node bin/genesis-lyric.mjs plan --brief-file <brief.json> --json` to normalize lyrics into a single-video plan.',
        '3. `node bin/genesis-lyric.mjs preflight --brief-file <brief.json> --json` before rendering.',
        '4. `node bin/genesis-lyric.mjs render --brief-file <brief.json> --approved --json` only after explicit approval.',
        '',
        'Briefs must provide `lyrics` or timed `lyric_lines`, plus one visual source as `video_file` or `image_file`. The visual can be user footage, user artwork, or an asset generated by another tool. Supported aspect ratios: `9:16`, `1:1`, `16:9`.',
        '',
