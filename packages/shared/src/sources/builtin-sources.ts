  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '6. Treat `browserPlan.accountVerification` as mandatory before final submit; `social execute` rejects handoffs without a known expected handle or account URL.',
        '7. Dry-run posts, comments, and DMs with `--asset-root`, `--content-root`, relative filenames, and `--dry-run --json` before live execution.',
        '8. Use the returned `RUNNER_CDP_DELEGATED` browser plan with Runner native browser tools, verify the visible account/channel, pause before final submit, and never use `--autorun` for write actions.',
        '',
