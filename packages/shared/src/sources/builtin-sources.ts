  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '6. Default engine is `runner-cdp`: save dry-run JSON as the action contract, then run `node src/social.mjs execute --action-file <dry-run-result.json> --expected-action-id <act_...> --confirm yes --json` after explicit approval. Spotify playlist dry-runs also require their exact `--expected-action-digest <sha256:...>`.',
