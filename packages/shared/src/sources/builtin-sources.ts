 ...
 * still belongs to the user because Instagram/TikTok/X/YouTube/Spotify sessions cannot
 * be pre-shipped.
 */
export function getPrintingPressSocialSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const config: FolderSourceConfig = {
    ...
    tagline: 'Direct-browser CLIs for Instagram, TikTok, X, YouTube, and Spotify channel work.',
    icon: '📣',
    isAuthenticated: true,
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '4. Resolve the exact account first. Users can name account sets like `MikeyReal` or profiles as `platform/profile`, such as `instagram/brand-main`; do not guess when multiple profiles exist.',
        '5. If campaign folders are involved, list assets with `node src/social.mjs assets --asset-root <dir> --platform <platform> --json` and text with `node src/social.mjs content --content-root <dir> --json`.',
        '6. Default engine is `runner-cdp`: save dry-run JSON as the action contract, then run guarded `social execute` after explicit approval. Spotify also requires the dry-run `approvalDigest` as `--expected-action-digest`.',
        '7. Treat `browserPlan.accountVerification` as mandatory before submit; `social execute` rejects handoffs without a known expected handle or account URL.',
        '8. Dry-run posts, comments, DMs, and Spotify playlist creation before live execution. Spotify snapshots are read-only capture plans.',
        '9. Use the returned `RUNNER_CDP_DELEGATED` browser plan with Runner native browser tools. If the visible account and draft match the approved dry-run, submit without asking again; stop only on mismatch, ambiguity, unexpected platform choices, or upload/UI failure.',
        '10. For Spotify, use `snapshot spotify` for read-only capture plans and dry-run `playlist spotify create` before approval. Never call playlist create directly with `--confirm`; use guarded execute with the exact id/digest, then record the observed URL through `playlist spotify receipt`.',
        '',
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '6. Use `setup-plan --platform meta --goal ... --artist-context <file> --territories "..." --budget "..." --campaign-name "..." --json` before browser-guided Meta Ads Manager draft setup.',
        '7. Use `packet create --platform meta|google --type publish|budget|status|targeting|creative --out packet.json --json` before any proposed write.',
