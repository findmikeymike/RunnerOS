  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '5. Default engine is `runner-cdp`: use CLI output as the action contract/plan, then execute through Runner native browser tools.',
        '6. Treat `browserPlan.accountVerification` as mandatory before final submit; verify the visible account/channel matches the expected handle or account URL.',
        '7. Dry-run posts, comments, and DMs with `--asset-root`, `--content-root`, relative filenames, and `--dry-run --json` before live execution.',
        '8. Ask for explicit approval before any live post, comment, or DM, then pass `--confirm yes` only for that approved action. Do not use `--autorun` for write actions.',
        '',
