  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '7. Default engine is `runner-cdp`: save dry-run JSON as the action contract, then run `node src/social.mjs execute --action-file <dry-run-result.json> --expected-action-id <act_...> --expected-action-digest <sha256:...> --confirm yes --json` after explicit approval. Every dry-run supplies its exact approval digest; pass that captured digest unchanged.',
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '',
        'Workflow:',
        '1. Use the connected native YouTube route first. If unavailable, use the pinned Monid metadata/transcript routes in the monid skill; Zero requires explicit user choice or a confirmed Monid capability gap; disconnection, budget blocks, outages, failures and unresolved paid attempts are not such a gap. A Data API key does not grant third-party transcript access.',
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        'YouTube is pinned: apify /starvibe/youtube-video-transcript for one timestamped transcript; apify /streamers/youtube-scraper for bounded channel/video metadata. Read the bundled monid skill. Inspect current schema and price, but do not repeat marketplace discovery. Host-managed Signals owns collection and paid-attempt receipts; do not duplicate it from an analyst session.',
        '',
        'Workflow:',
        '1. Use a matching pin from the monid or domain skill; otherwise discover a focused capability through the connected MCP tools. Read their actual schemas and never invent MCP tool names.',
        ...
        '- Use Zero only for explicit user choice or a confirmed missing Monid capability. Disconnection, budget blocks, outages, failures and unresolved paid calls are not capability absence. Never repeat an unresolved charge through another provider.',
      ].join('\n'),
    },
    isBuiltin: true,
  };
...
export function getZeroSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const config: FolderSourceConfig = {
    ...
    tagline: 'Guarded Zero tools for explicit user choice or a confirmed Monid capability gap.',
    icon: '0',
  ...
  return {
    ...
    guide: {
      raw: [
        ...
        '6. Locate the installed guard with `craft-agent skill where zero`, then check its weekly allowance with `node <zero-skill-dir>/scripts/zero-budget.mjs status --json`.',
        '7. GET retrieval runs through the weekly guard without another prompt. Non-GET work requires one bounded job authorization reused across the approved batch or workflow.',
        '8. Run guarded calls with `node <zero-skill-dir>/scripts/zero-budget.mjs fetch --capability <exact-slug> --max-pay <usd> --json`; add the approved authorization ID for non-GET work.',
        '9. Review paid calls with `zero review <runId> --accuracy N --value N --reliability N --content "<specific observation>"`.',
        '',
        'Hard rules:',
        ...
        '- Weekly allowance controls spend; a bounded capability/method/call-count/total/expiry receipt controls non-GET work without per-call prompts.',
        '- Use `--no-open` for funding URLs inside agents; hand the URL to the user.',
        '- Ask before funding wallets, installing CLIs, setting/changing the weekly allowance, or creating/changing a bounded job authorization.',
      ].join('\n'),
    },
    isBuiltin: true,
  };
