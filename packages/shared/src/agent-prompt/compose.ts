...
export function composeAgentSystemPrompt(
  ...
  if (memory.artistWorkspaceScope === 'hq' || memory.artistWorkspaceScope === 'campaign' || memory.artistWorkspaceScope === 'lab') {
    ...
    if (agent.slug === 'content-genius') parts.push(CONTENT_GENIUS_COLLABORATION_GUIDANCE);
    const websiteGuidance = buildWebsiteCampaignGuidance(agent.slug, memory.artistWorkspaceScope);
...
export function buildArtistAssetContractSection(
  ...
  return [
    ...
    '- Working Outputs do not have automatic same-ID revision history or an update_output tool. Editing their source in place changes the preview without retaining old bytes or refreshing recorded file metadata; never claim it preserves version continuity. To preserve history, keep the original, write a new revision file, and create a new Output with an explained new ID. If the user requires both the same ID and preserved history, explain that unsupported combination before editing and resolve the conflict rather than silently creating a second Output. Honor explicit in-place edits while disclosing that the prior version is not automatically retained. Release Kit snapshots remain independent immutable copies.',
