...
describe('artist vault', () => {
  ...
  test('async import keeps supplied notes and uses a readable filename for blank labels', async () => {
    const workspace = tempWorkspace();
    const source = join(workspace, 'my_press-photo.png');
    writeFileSync(source, 'image');
    const result = await importArtistVaultAssetsAsync(workspace, 'workspace-1', [source], {
      details: [{ sourcePath: source, label: '  ', notes: '  Approved press crop  ' }],
    });
    expect(result.imported[0]?.label).toBe('my press photo');
    expect(result.imported[0]?.notes).toBe('Approved press crop');
    const saved = loadArtistVaultManifest(workspace, 'workspace-1').assets[0];
    expect(saved?.notes).toBe('Approved press crop');
  });

  test('rejects large Vault previews before reading the file into memory', async () => {
    const workspace = tempWorkspace();
    const source = join(workspace, 'large.mp4');
    writeFileSync(source, 'video');
    const asset = importArtistVaultAssets(workspace, 'workspace-1', [source], { kindHint: 'raw-footage' }).imported[0]!;
    const { truncateSync } = await import('node:fs');
    truncateSync(join(workspace, asset.relativePath!), 33 * 1024 * 1024);
    await expect(readArtistVaultAssetDataUrl(workspace, 'workspace-1', asset.id)).rejects.toThrow('too large for inline preview');
  });
