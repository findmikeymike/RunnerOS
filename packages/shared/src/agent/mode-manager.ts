...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  const workspacesMarker = normalizedPlans.indexOf('/workspaces/');
  const currentRuntimeWorkspacesRoot = workspacesMarker >= 0
    ? normalizedPlans.slice(0, workspacesMarker + '/workspaces/'.length)
    : null;
  ...
  if (currentRuntimeWorkspacesRoot && normalizedTarget.startsWith(currentRuntimeWorkspacesRoot) && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside the current product runtime entirely
  if (currentRuntimeWorkspacesRoot && !normalizedTarget.startsWith(currentRuntimeWorkspacesRoot)) {
