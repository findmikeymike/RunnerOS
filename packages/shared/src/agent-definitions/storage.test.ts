...
`, 'utf-8')
  ...
  test('starter library includes the Lyric Video Agent with Genesis lyric source', () => {
    ...
    expect(lyricVideoAgent?.systemPrompt).toContain('storyboard the lyric section in chat')
    expect(lyricVideoAgent?.systemPrompt).toContain('Think in shots, not just one prompt')
