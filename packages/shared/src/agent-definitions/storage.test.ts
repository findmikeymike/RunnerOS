...
`, 'utf-8')
  ...
  test('starter library includes Video Director with Squad source and artifact-window publishing', () => {
    ...
    expect(videoDirector?.systemPrompt).toContain('node <squad-source-local-path>/bin/squad.mjs storyboard')
    expect(videoDirector?.systemPrompt).not.toContain('node tools/squad/bin/squad.mjs storyboard')
