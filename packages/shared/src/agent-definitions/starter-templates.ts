...
const BRANDING_TASK_MODES: AgentTaskModeDefinition[] = [
  ...
  },
  {
    ...
    adjacentSkills: [
      { slug: 'artist-brand-dna-audit', when: 'Use when conflicting evidence needs an identity diagnosis first.', expansion: 'same-session' },
      { slug: 'artist-belief-system', when: 'Use when the world lacks a clear belief or reason for the tribe to gather.', expansion: 'same-session' },
    ...
    context: {
      ...
      retrieveOnDemandTopics: ['lyrics and demos', 'approved Vault references', 'past visual campaigns', 'current asset inventory'],
      maxPreloadChars: 12_000,
    },
  },
  {
    ...
    adjacentSkills: [
      { slug: 'artist-narrative-universe', when: 'Use when the beliefs need a story world or mythology.', expansion: 'same-session' },
      { slug: 'artist-visual-world-director', when: 'Use when public expression needs a visual direction.', expansion: 'same-session' },
    ...
    context: {
      preloadTopics: ['artist-profile', 'artist-voice', 'artist-branding'],
      retrieveOnDemandTopics: ['lyrics and interviews', 'recent posts and captions', 'fan comments', 'community evidence'],
      maxPreloadChars: 12_000,
    },
  },
  {
...
not facts about the user.`,
  },
  {
    ...
    systemPrompt: `You are Setup Concierge, Artist OS's app helper. Explain how the current app works, find the right capability, and help with setup. Artist Manager owns ongoing priorities and delegation; specialists own their domain work.
...
10. After exact-action approval or when a reply fits an active bounded engagement mandate, save the dry-run result and run \`node src/social.mjs execute --action-file <dry-run-result.json> --expected-action-id <act_...> --expected-action-digest <sha256:...> --confirm yes --json\`. Treat the returned \`RUNNER_CDP_DELEGATED\` result as the guarded Runner browser handoff.
