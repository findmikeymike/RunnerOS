...
describe('system prompt guidance', () => {
  ...
  it('teaches agents when to use message_agent for specialist delegation', () => {
    ...
    expect(prompt).toContain('source/account readiness')
    expect(prompt).toContain('Do not route to a blocked specialist')
    expect(prompt).toContain('use a degraded specialist only when')
    expect(prompt).toContain('Do not paste the whole transcript')
    expect(prompt).toContain('Give each specialist one complete bounded handoff')
    expect(prompt).toContain('Use multiple specialists only for genuinely distinct roles')
    expect(prompt).toContain('Never delegate to your own slug')
