...
function copyPrivateDirectoryVerified(
  sourceDirectory: string,
  destinationDirectory: string,
  sourceRootPath: string,
  destinationRootPath: string,
): void {
  assertPathWithinRealRoot(sourceRootPath, sourceDirectory);
  const sourceStat = lstatSync(sourceDirectory);
  if (sourceStat.isSymbolicLink() || !sourceStat.isDirectory()) {
    throw new Error(`Team Mode private copy requires a regular directory: ${sourceDirectory}`);
  }

  mkdirSync(destinationDirectory, { mode: 0o700 });
  assertPathWithinRealRoot(destinationRootPath, destinationDirectory);
  ...
  for (const entry of readdirSync(sourceDirectory, { withFileTypes: true })) {
    ...
    if (current.isSymbolicLink()) {
      throw new Error(`Team Mode private copy refused a symbolic link: ${sourcePath}`);
    }
    if (current.isDirectory()) {
      copyPrivateDirectoryVerified(sourcePath, destinationPath, sourceRootPath, destinationRootPath);
    } else if (current.isFile()) {
      verifiedCopyFileSync(sourcePath, destinationPath, { sourceRootPath, destinationRootPath });
    } else {
      throw new Error(`Team Mode private copy refused a special file: ${sourcePath}`);
    }
  }
...
function quarantineInPlacePrivateArtifacts(workspaceRootPath: string, workspaceId: string): void {
  ...
  const privateTeamRoot = getPrivateTeamDir(workspaceId);
  const privateSessions = join(privateTeamRoot, 'private-sessions');
  if (existsSync(sourceSessions)) {
    ...
    mkdirSync(privateTeamRoot, { recursive: true, mode: 0o700 });
    rmSync(privateSessions, { recursive: true, force: true });
    try {
      copyPrivateDirectoryVerified(sourceSessions, privateSessions, workspaceRootPath, privateTeamRoot);
    } catch (error) {
      rmSync(privateSessions, { recursive: true, force: true });
      throw error;
    }
