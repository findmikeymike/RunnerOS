...
test('schema four upgrades while preserving old local run contents', () => {
  ...
  expect((f.journal as any).db.prepare('PRAGMA user_version').get().user_version).toBe(8); expect(f.journal.get('r', 'w').spec.allowedTools).toEqual(['read']);
});
...
test('schema five web reads migrate with redirects still disabled by default', () => {
  ...
  expect((f.journal as any).db.prepare('PRAGMA user_version').get().user_version).toBe(8);
