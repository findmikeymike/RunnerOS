...
describe('resource-bundle', () => {
  ...
  describe('exportResources', () => {
    ...
    it('exports automations as per-entry array', () => {
      ...
      expect(greeting.event).toBe('SchedulerTick')
  ...
  describe('validateResourceBundle', () => {
    ...
    it('accepts valid automation entries', () => {
      const bundle = {
        ...
        resources: {
          automations: [
            { id: 'aaa111', event: 'SchedulerTick', matcher: { id: 'aaa111', actions: [{ type: 'prompt', prompt: 'hi' }] } },
          ],
        },
      }
    ...
    it('rejects duplicate automation IDs', () => {
      const bundle = {
        ...
        resources: {
          automations: [
            { id: 'aaa111', event: 'SchedulerTick', matcher: { id: 'aaa111', actions: [{ type: 'prompt', prompt: 'a' }] } },
    ...
    it('allows duplicate automation names', () => {
      const bundle = {
        ...
        resources: {
          automations: [
            { id: 'aaa111', name: 'Same Name', event: 'SchedulerTick', matcher: { id: 'aaa111', actions: [{ type: 'prompt', prompt: 'a' }] } },
    ...
    it('rejects automation without actions', () => {
      const bundle = {
        ...
        resources: {
          automations: [
            { id: 'aaa111', event: 'SchedulerTick', matcher: { id: 'aaa111', actions: [] } },
          ],
        },
      }
  ...
  describe('importResources', () => {
    ...
    it('imports automations into workspace with no existing file', async () => {
      ...
      const bundle: ResourceBundle = {
        ...
        resources: {
          automations: [
            makeAutomationEntry({ id: 'aaa111', name: 'Auto 1', event: 'SchedulerTick' }),
          ],
        },
      }
      ...
      expect(config.automations.SchedulerTick[0].id).toBe('aaa111')
    })
    ...
    it('merges automations into existing config', async () => {
      ...
      expect(config.automations.SchedulerTick).toHaveLength(1)
      expect(config.automations.SchedulerTick[0].id).toBe('existing1')
    ...
    it('skips automations with existing ID in skip mode', async () => {
      ...
      const bundle: ResourceBundle = {
        ...
        resources: {
          automations: [
            makeAutomationEntry({ id: 'aaa111', name: 'Updated', event: 'SchedulerTick' }),
          ],
        },
      }
      ...
      const config = JSON.parse(readFileSync(join(wsDir, 'automations.json'), 'utf-8'))
      expect(config.automations.SchedulerTick[0].name).toBe('Original')
    })
    ...
    it('overwrites automation by ID in overwrite mode', async () => {
      ...
      const bundle: ResourceBundle = {
        ...
        resources: {
          automations: [
            makeAutomationEntry({ id: 'aaa111', name: 'Replaced', event: 'SchedulerTick' }),
          ],
        },
      }
      ...
      const config = JSON.parse(readFileSync(join(wsDir, 'automations.json'), 'utf-8'))
      // Replaced automation
      const names = config.automations.SchedulerTick.map((m: any) => m.name)
    ...
    it('selectively clears history and retry queue for overwritten IDs', async () => {
      ...
      const bundle: ResourceBundle = {
        ...
        resources: {
          automations: [
            makeAutomationEntry({ id: 'aaa111', event: 'SchedulerTick' }),
          ],
        },
      }
    ...
    it('starts fresh when existing automations.json is invalid in overwrite mode', async () => {
      ...
      const bundle: ResourceBundle = {
        ...
        resources: {
          automations: [
            makeAutomationEntry({ id: 'aaa111', name: 'Fresh Start', event: 'SchedulerTick' }),
          ],
        },
      }
  ...
  describe('round-trip export → import', () => {
    ...
    it('preserves automations through round-trip', async () => {
      ...
      createTestAutomations(srcDir, {
        // Two different events on purpose: the round-trip has to preserve
        // automations across more than one trigger, not collapse them.
        MessageReceive: [
      ...
      expect(config.version).toBe(2)
      expect(config.automations.MessageReceive).toHaveLength(1)
      expect(config.automations.MessageReceive[0].name).toBe('Greet')
      expect(config.automations.SchedulerTick).toHaveLength(1)
