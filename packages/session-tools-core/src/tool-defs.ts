...
import { handleSaveReleaseCreativeBrief, saveReleaseCreativeBriefSchema } from './handlers/save-release-creative-brief.ts';
...
import { handleSetupLlmConnection, setupLlmConnectionSchema } from './handlers/setup-llm-connection.ts';
import { handleSetupSocialAccount, setupSocialAccountSchema } from './handlers/setup-social-account.ts';
...
Use only after the user explicitly confirms the operation. Read the current coordinator first and pass its exact runId and updatedAt. Re-arm must restate the objective; increase maxRounds only when the user approves a new bound. Never use this tool to bypass an approval or resume an uncertain external action.`,
  ...
  website_inspect_external: `Read the artist's existing site — the Squarespace, Wix, WordPress or Bandcamp page they already have — and remember what is on it. Returns the platform, every page found with its title, whether a visitor can leave an email address anywhere and where those addresses currently go, and what is worth telling the artist. Read-only: it fetches and parses, and changes nothing on the site. Call this before answering any question about their existing site or offering to change it; the answer includes where the artist logs in to edit it, which is how any change actually gets made — there is no API path for these sites. The reading is stored, so later calls answer from memory — pass \`refresh: true\` when you need to see whether something has actually changed.`,
...
export const SESSION_TOOL_DEFS: SessionToolDef[] = [
  ...
  { name: 'manage_artist_brain', description: 'Read or update one HQ Brain topic: profile, voice (writing style, not call audio), or branding. Read first to get allowed field names and revision; update only discussed artist-supplied or approved fields, using that exact expectedRevision. null explicitly clears a field. Preserves all other information. Direct Setup Concierge/Artist Manager only; no automatic career research.', inputSchema: manageArtistBrainSchema, executionMode: 'registry', safeMode: 'block', handler: handleManageArtistBrain },
  { name: 'propose_branding_update', description: 'Prepare specific Artist HQ DNA changes or useful supporting documents for artist review in Brain → Branding. Artist Direction in HQ only. Read artist-branding first and provide exact current field values. Nothing becomes shared context until the artist clicks Apply to Branding. Never claim a pending proposal is approved or saved as current DNA.', inputSchema: proposeBrandingUpdateSchema, executionMode: 'registry', safeMode: 'block', handler: handleProposeBrandingUpdate },
  { name: 'save_release_creative_brief', description: 'Save Release Creative Direction in the current campaign for World Builder and production agents. Creative Direction only. Read campaign-creative-direction with get_workspace_context first; pass its exact full body as expectedBody, or null for first creation. Explicitly distinguish proposed from artist-accepted direction. Does not change HQ identity, publish, schedule, or grant execution approval.', inputSchema: saveReleaseCreativeBriefSchema, executionMode: 'registry', safeMode: 'block', handler: handleSaveReleaseCreativeBrief },
  { name: 'import_artist_community', description: 'Import user-supplied fan/audience contacts into HQ Community, not professional Network. At most 100 per batch. Reuses Community records, skips duplicates and suppressed/deleted people, never overwrites consent. Email alone is not opt-in: omit consent unless the user supplied explicit permission and source evidence. No sending or external sync. Report row receipts and counts. Ask destination once if Network versus Community is unclear.', inputSchema: importArtistCommunitySchema, executionMode: 'registry', safeMode: 'block', handler: handleImportArtistCommunity },
  { name: 'import_artist_network', description: 'Add user-supplied people to the global Artist HQ Network from pasted notes or an attached file. Extract only stated names, emails, roles and notes; omit missing fields, flag unreadable/invalid emails instead of inventing them. Up to 100 per batch. Checks duplicates and reports ambiguous matches without overwriting existing contacts. No sending, fan subscriptions or Google sync. Use directly in Setup Concierge or Artist Manager when the user asks to add people; no chat handoff is needed. Report added/existing/needsClarification counts from the receipt.', inputSchema: importArtistNetworkSchema, executionMode: 'registry', safeMode: 'block', handler: handleImportArtistNetwork },
  { name: 'setup_llm_connection', description: 'Inspect saved model connections, open the secure Settings wizard to add or reconnect one, test a saved connection, or explicitly select an app/workspace default. Never send credentials in tool arguments. A saved credential is not a successful provider test. Available to interactive Setup Concierge and Artist Manager.', inputSchema: setupLlmConnectionSchema, executionMode: 'registry', safeMode: 'block', handler: handleSetupLlmConnection },
  { name: 'setup_social_account', description: 'List, add, open, or verify saved social browser accounts and Spotify services using the same profiles as Settings. Login happens personally in the controlled browser. Spotify open/verify requires the surface; select the intended artist before verifying Artists. Historical verification is not fresh authentication. Never pass passwords, cookies, or session tokens.', inputSchema: setupSocialAccountSchema, executionMode: 'registry', safeMode: 'block', handler: handleSetupSocialAccount },
