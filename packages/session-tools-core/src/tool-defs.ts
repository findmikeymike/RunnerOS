...
export const CreateOutputSchema = z.object({
  ...
  context: z.object({
    scope: z.enum(['hq', 'campaign']).describe('Where this Work Product should appear. Use hq for HQ-level work, campaign for campaign-specific work.'),
    campaignId: z.string().min(1).optional().describe('Required when scope is campaign; omit for hq.'),
  }).optional().describe('Small routing context for HQ/campaign Work Products.'),
  approval: z.object({
    state: z.enum(['none', 'pending', 'approved', 'changes_requested']).describe('Use pending only when the user needs to approve, reject, or request changes.'),
    note: z.string().optional(),
    updatedAt: z.string().optional().describe('ISO-8601 timestamp. Usually omitted; backend/UI can set it.'),
  }).optional().describe('User-decision state for this Output.'),
...
When the deliverable should appear on HQ or a campaign dashboard, set \`context.scope\` to \`hq\` or \`campaign\`. For campaign work, include \`context.campaignId\`.

Set \`approval.state: "pending"\` only when the user needs to approve, reject, or request changes. Otherwise omit approval or use \`approval.state: "none"\`.
