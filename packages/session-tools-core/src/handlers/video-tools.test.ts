...
import { existsSync, mkdirSync, mkdtempSync, readFileSync, rmSync, writeFileSync, symlinkSync, linkSync } from 'node:fs';
...
describe('video studio session tools', () => {
  ...
  test('video_inspect_timeline renders JPEG frame paths from the actual timeline', async () => {
    expect(ffmpegAvailable && ffprobeAvailable).toBe(true);
    const ctx = makeCtx();
    const projectPath = join(root, 'project', 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, {
      projectPath,
...
describe('export integrity regressions', () => {
  test.each([0.5, 2])('split preserves source continuity at %sx speed', async (speed) => {
    const ctx = makeCtx();
    const projectPath = join(root, 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, { projectPath, title: 'Speed' });
  ...
  test.each(['symlink', 'hardlink', 'receipt'])('refuses source overwrite through %s alias', async (kind) => {
    const ctx = makeCtx();
    const projectPath = join(root, 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, { projectPath, title: 'Safety' });
    const source = join(root, 'source.mov');
    writeFileSync(source, 'original media');
    const outputPath = join(root, 'export.txt');
    if (kind === 'symlink') symlinkSync(source, outputPath);
    if (kind === 'hardlink') linkSync(source, outputPath);
    const project = JSON.parse(readFileSync(projectPath, 'utf8'));
    project.media = [{ id: 'media', label: 'Source', type: 'video', path: kind === 'receipt' ? outputPath + '.receipt.json' : source }];
    if (kind === 'receipt') writeFileSync(outputPath + '.receipt.json', 'original media');
    writeFileSync(projectPath, JSON.stringify(project));
    const result = await handleVideoExport(ctx, { projectPath, outputPath });
    expect(result.isError).toBe(true);
    expect(JSON.stringify(result.content)).toContain('Refusing to overwrite source media');
    expect(readFileSync(project.media[0].path, 'utf8')).toBe('original media');
  });

  test('fractional second export retains timeline duration', async () => {
    expect(ffmpegAvailable && ffprobeAvailable).toBe(true);
    const ctx = makeCtx();
    const projectPath = join(root, 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, { projectPath, title: 'Duration', width: 320, height: 240, fps: 30 });
...
describe('complete text rendering', () => {
  ...
  test('renders ninth text clip and preserves text after 180 characters', async () => {
    expect(ffmpegAvailable && ffprobeAvailable).toBe(true);
    const ctx = makeCtx();
    const projectPath = join(root, 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, { projectPath, title: 'Text' });
    const project = JSON.parse(readFileSync(projectPath, 'utf8'));
    ...
    for (let offset = 320 * 1120 * 3; offset < frame.stdout.length; offset += 3) {
      if ((frame.stdout[offset] ?? 0) > 200 && (frame.stdout[offset + 1] ?? 0) > 200 && (frame.stdout[offset + 2] ?? 0) > 200) brightPixels++;
    }
...
describe('export preserves project control files', () => {
  test.each(['project', 'backup', 'lock', 'undo', 'symlink', 'hardlink', 'receipt-alias', 'dangling-lock-alias', 'dot-prefix-undo', 'undo-hardlink'])('rejects %s destination before writing', async (kind) => {
    const ctx = makeCtx();
    const projectPath = join(root, 'video.runner-video.json');
    await handleVideoProjectCreate(ctx, { projectPath, title: 'Preserve project' });
    ...
    const undo = join(root, '.runner-video', 'undo', 'previous.runner-video.json');
    mkdirSync(dirname(undo), { recursive: true }); writeFileSync(undo, 'undo snapshot');
    let outputPath = join(root, 'export.txt');
    if (kind === 'project') outputPath = projectPath;
    if (kind === 'backup') outputPath = backup;
    if (kind === 'lock') { outputPath = lock; writeFileSync(lock, 'live lock'); }
    if (kind === 'undo') outputPath = undo;
    if (kind === 'dot-prefix-undo') outputPath = join(dirname(dirname(undo)), '..foo');
    if (kind === 'undo-hardlink') linkSync(undo, outputPath);
    if (kind === 'symlink') symlinkSync(projectPath, outputPath);
    if (kind === 'hardlink') linkSync(projectPath, outputPath);
    if (kind === 'receipt-alias') symlinkSync(projectPath, outputPath + '.receipt.json');
    if (kind === 'dangling-lock-alias') symlinkSync(lock, outputPath);
    const result = await handleVideoExport(ctx, { projectPath, outputPath });
    expect(result.isError).toBe(true);
    expect(JSON.stringify(result.content)).toContain('Refusing to overwrite video project or recovery files');
