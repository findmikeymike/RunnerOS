...
export class DurableTurnController {
  ...
  constructor(
    ...
    if (descriptor.engine !== 'sqlite-v2-readonly-1' || !Number.isSafeInteger(descriptor.createdAt)
      ...
      || descriptor.allowedTools.some(name => !['read', 'grep', 'find', 'ls'].includes(name))) {
  ...
  async run(agent: Agent, message: string, systemPrompt: string): Promise<void> {
    ...
    const unsubscribe = agent.subscribe(async event => {
      ...
      if (event.type === 'tool_execution_end' && event.isError) this.fail(new Error(`SDK tool failed: ${event.toolName}`));
    });
    try {
      await agent.prompt({ role: 'user', content: [{ type: 'text', text: message }], timestamp: this.descriptor.createdAt });
      this.assertHealthy();
    ...
    finally { unsubscribe(); agent.streamFunction = originalStream; }
  }
}
