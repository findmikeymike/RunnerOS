...
The backend may reply with either JSON or SSE-like payloads depending on edge behavior. The provider parses both formats. Do not retry with `web_search_preview` for ChatGPT-backed Codex accounts; smoke testing on 2026-07-07 showed the backend rejects it with `Unsupported tool type: web_search_preview`. Higher-level search falls back to DuckDuckGo when the ChatGPT provider fails.
...
3. Confirm model remains supported by ChatGPT-backed Codex accounts.
