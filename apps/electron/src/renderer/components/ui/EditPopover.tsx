...
import { productDataPath, productDeepLink } from '@/lib/product-identity'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        `The user is editing app-level default permissions (${productDataPath('permissions/default.json')}). ` +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        `The user is editing the notes field in their preferences (${productDataPath('preferences.json')}). ` +
  ...
  'add-source': (location) => ({
    context: {
      ...
      context:
        ...
        `Follow the patterns in ${productDataPath('docs/sources.md')}. ` +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        `Follow the patterns in ${productDataPath('docs/skills.md')}. ` +
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        `Read ${productDataPath('docs/labels.md')} for full format reference. ` +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        `The file is tool-icons.json in ${productDataPath('tool-icons')}. Icon image files live in the same directory. ` +
        ...
        `Read ${productDataPath('docs/tool-icons.md')} for full format reference. ` +
  ...
  'automation-config': (location) => ({
    context: {
      ...
      context:
        ...
        `Read ${productDataPath('docs/automations.md')} for full format reference. ` +
...
export function EditPopover({
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = productDeepLink(`action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`)
