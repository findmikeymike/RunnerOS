...
export function ReleaseCountdownDial({
  ...
  return (
    <button
      ...
      className="group relative flex h-[136px] w-[136px] shrink-0 items-center justify-center rounded-full border border-white/[0.09] bg-gradient-to-br from-[#1b1b1b] via-[#0f0f0f] to-[#020202] p-2 shadow-strong transition-transform hover:scale-[1.02] active:scale-[0.99]"
    >
      <span className="absolute inset-2 rounded-full bg-[#030303] shadow-panel-focused" />
      <span
        className="absolute inset-3 rounded-full opacity-95 transition-opacity group-hover:opacity-100"
        style={{
      ...
      />
      <span className="relative z-10 flex h-[92px] w-[92px] flex-col items-center justify-center rounded-full border border-white/[0.075] bg-gradient-to-br from-[#181818] to-[#070707] shadow-strong">
        <span className="absolute inset-2 rounded-full border border-white/[0.035]" />
        <span className="absolute left-1/2 top-2 h-1.5 w-1.5 -translate-x-1/2 rounded-full bg-black shadow-xs" />
        <span className={cn(
          'relative z-10 font-light tracking-[-0.06em] text-[#ff9700]',
          countdown.released ? 'text-[18px] tracking-[0.05em]' : 'text-[35px]',
        )}>
          {value}
        </span>
        <span className="relative z-10 text-[7px] font-semibold uppercase tracking-[0.16em] text-white/64">{label}</span>
        <span className="relative z-10 mt-0.5 text-[8px] font-medium text-white/34">{countdown.dateLabel}</span>
      </span>
    </button>
  )
}
