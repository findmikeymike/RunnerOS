...
describe('Artist OS persistent shell chrome', () => {
  ...
  test('keeps Command conversation history bounded, newest-first, and topic-labelled', () => {
    ...
    expect(shell).toContain('event.currentTarget.scrollLeft = 0')
  })
  ...
  test('starts mode-aware workers inside chat with a compact focus row', () => {
    ...
    expect(workers).not.toContain('shouldDeferAgentTaskModeSelection(agent)')
    ...
    expect(chatPage).toContain('startConversation: false')
    expect(chatPage).toContain('const selectedTaskModeId = confirmedTaskModeId ?? receiptTaskModeId')
    expect(chatPage).toContain('conversationStarted={conversationStarted}')
    expect(chatPage).toContain('openingConversation={openingTaskModeConversation}')
    expect(chatPage).not.toContain('&& !hasMessages')
    expect(chatPage).toContain('disableSend={Boolean(applyingTaskModeId)}')
    expect(chatPage).toContain('focusControls={showTaskModeBar ? (')
    expect(focusBar).toContain('next reply')
    ...
    expect(runAgent).not.toContain('taskModeSelectionPending: true')
    expect(runAgent).toContain('GENERAL_AGENT_TASK_MODE_ID')
  })
  ...
  test('keeps Artist OS chat and notification text visible on black surfaces', () => {
    ...
    expect(agentHeader).toContain('font-semibold uppercase')
  ...
  test('orders Spotify before Social and keeps Social focused on Instagram analytics', () => {
    ...
    expect(hq).toContain("{ label: 'Reach', value: snapshot?.metrics.accountsReached }")
    expect(hq).toContain("{ label: 'Interactions', value: snapshot?.metrics.interactions }")
