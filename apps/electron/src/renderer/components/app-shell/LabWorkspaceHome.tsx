...
const RECENT_SONGS = [
  { title: 'Untitled night-drive hook', detail: 'Loose Singles · focused · today' },
  { title: 'Pretty trouble', detail: 'EP One · focused · yesterday' },
  { title: 'Backseat prophecy', detail: 'Loose Singles · working · 3d ago' },
]

const SPARKS = [
  'The exhaustion of always being perceived',
  'Soft rebellion after burnout',
  'Luxury as armor',
]
...
export function LabWorkspaceHome({ workspaceName }: LabWorkspaceHomeProps) {
  return (
    <div className="h-full overflow-y-auto bg-[#050505] text-foreground">
      <div className="flex w-full flex-col gap-3 px-5 py-4 xl:px-8 xl:py-5">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <LabCard>
            <SectionTitle icon={Clock3} title="Recent Songs" meta="3" />
            <div className="space-y-2">
              {RECENT_SONGS.map((song) => (
                ...
                >
                  ...
                  <p className="mt-1 truncate text-xs text-white/36">{song.detail}</p>
                </button>
              ))}
            </div>
          </LabCard>
          ...
          <LabCard>
            ...
            <div className="rounded-xl border border-white/[0.03] bg-white/[0.012] p-4">
              <p className="text-sm font-medium text-white/75">No worker running</p>
              <p className="mt-1 text-xs leading-5 text-white/38">
                Writer, Researcher, and Record Doctor are ready when you need them.
              </p>
              <button
                type="button"
