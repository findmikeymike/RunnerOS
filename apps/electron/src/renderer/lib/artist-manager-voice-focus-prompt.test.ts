...
describe('focused voice prompt', () => {
  ...
  test('Mikey adds personality while preserving the bounded artist brief and conversational intent', () => {
    ...
    expect(prompt).toContain('Command confirmation only opens an unsent draft')
    expect(prompt).toContain('Background draft confirmation starts bounded specialist work')
    expect(prompt).not.toContain('Use open_command_chat for an agreed task the artist wants to act on')
  ...
  test('excludes unrelated context, skill bodies, and worker catalog while placing delivery boundaries after the snapshot', () => {
    ...
    expect(tail).toContain('report its outcome only from host evidence')
