...
import type { ArchiveSocialVariantRequest, CreateSocialVariantSetRequest, OutputFinalPointer as OutputFinalPointerDTO, OutputManifest as OutputManifestDTO, OutputSummary as OutputSummaryDTO, RebindSocialVariantSetRequest, StartSocialVariantSetRequest } from '@craft-agent/shared/outputs';
export type { ArchiveSocialVariantRequest, CreateSocialVariantSetRequest, OutputFinalPointerDTO, OutputManifestDTO, OutputSummaryDTO, RebindSocialVariantSetRequest, StartSocialVariantSetRequest };
...
export interface ElectronAPI {
  ...
  archiveSocialVariant(workspaceId: string, input: ArchiveSocialVariantRequest): Promise<OutputManifestDTO>
  rebindSocialVariantSet(workspaceId: string, input: RebindSocialVariantSetRequest): Promise<OutputManifestDTO>
  ...
  readOutputAssetMediaInfo(workspaceId: string, outputId: string, assetId: string, expectedSourcePath: string): Promise<{ hasAudio: boolean }>
  readOutputAssetDataUrl(workspaceId: string, outputId: string, assetId?: string, expectedSourcePath?: string): Promise<string>
  ...
  artistManagerVoiceFocus: {
    ...
    cancel(request: VoiceFocusCancelRequest): Promise<void>
    stop(sessionId?: string): Promise<void>
    onEvent(callback: (event: VoiceFocusEvent) => void): () => void
  }
  ...
  getAutomationHistory(workspaceId: string, automationId: string, limit?: number): Promise<Array<{ id: string; ts: number; ok: boolean; sessionId?: string; prompt?: string; workOrderIds?: string[]; workTitle?: string; error?: string; webhook?: { method: string; url: string; statusCode: number; durationMs: number; attempts?: number; error?: string; responseBody?: string } }>>
  ...
  getMessagingBindings(): Promise<Array<{ id: string; workspaceId: string; agentSlug: string; activeSessionId?: string; platform: string; channelId: string; channelName?: string; enabled: boolean; createdAt: number }>>
