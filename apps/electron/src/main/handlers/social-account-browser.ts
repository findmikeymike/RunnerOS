...
export function hasLoggedInSignal(platform: string, text: string, currentUrl: string): boolean {
  ...
  if (platform === 'instagram') {
    return url?.hostname === 'www.instagram.com' && /\/(direct|accounts\/edit|create)(?:[/?#]|$)/i.test(pathname)
      || /\b(home|messages|notifications|create|profile)\b/i.test(text)
  }
  if (platform === 'x') {
    return url?.hostname === 'x.com' && /\/(compose|home|messages|notifications|settings)(?:[/?#]|$)/i.test(pathname)
      || /\b(post|messages|notifications|premium)\b/i.test(text)
  }
  if (platform === 'tiktok') {
    return url?.hostname === 'www.tiktok.com' && /\/(upload|messages|setting|creator-center)(?:[/?#]|$)/i.test(pathname)
      || /\b(upload|messages|profile|following)\b/i.test(text)
  }
  if (platform === 'youtube') {
    return url?.hostname === 'www.youtube.com' && /\/(feed|account|channel|@|upload|studio)(?:[/?#]|$)/i.test(pathname)
