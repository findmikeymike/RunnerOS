...
function fixture() {
  ...
  const options = { installationId, server: { isAuthenticatedClientConnected: (id: string) => clients.has(id) }, getBinding: () => binding, getWorkspaces: () => workspaces };
  return { resolve: createLocalDurableWorkflowAuthority(options), options, clients, binding, removeWorkspace: () => workspaces = [] };
}
