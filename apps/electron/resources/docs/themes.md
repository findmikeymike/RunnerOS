...
3. **Preset themes**: `~/.trade-god/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.trade-god/theme.json` - Override specific colors (app-level)
...
```
~/.trade-god/workspaces/{id}/config.json
```
...
Create `~/.trade-god/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.trade-god/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.trade-god/themes/{name}.json`
...
1. Create `~/.trade-god/theme.json` for overrides or `~/.trade-god/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.trade-god/theme.json` for overrides, `~/.trade-god/themes/` for presets)
