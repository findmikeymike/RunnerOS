...
copy_claude_sdk() {
    local native_package="claude-agent-sdk-darwin-${ARCH}"
    ...
    require_path "$native_source" "Claude native SDK package (${native_package})" "Run 'bun install' for darwin-${ARCH} from the repository root first."
...
echo "=== Building Trade God DMG (${ARCH}) using electron-builder ==="
if [ "$UPLOAD" = true ]; then
...
# electron-builder.yml uses artifactName to output: Trade-God-${arch}.dmg
DMG_NAME="Trade-God-${ARCH}.dmg"
