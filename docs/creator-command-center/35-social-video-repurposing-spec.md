...
- Add an optional typed `socialVariantSet` field under the existing Output schema. Its nested schema is independently versioned, so old Outputs remain valid without rewriting media or reserving a new outer version for one subtype.
