...
| R13 Explicit reset | HQ normally represents one artist; do not add automatic artist switching. An intentional repurpose requires **Clear context** before a stable identity can change. | A13 Clear stops active research, removes old findings/corrections from agent use, preserves Profile/Branding/Voice/growth, warns old chats not to reuse cleared facts, and allows clean reseeding. Standalone Outputs are disclosed as retained. |
...
Identity change (Spotify artist ID or official identity anchor) is rejected after research context exists until the user explicitly chooses **Clear context**. Clear does not invent a multi-artist switcher or automatically archive/restore identities. It privacy-scrubs the canonical context record, leaves a fact-free invalidation marker for existing chats, then lets the user reseed from saved Profile data or new public links. That marker remains always-delivered through setup, failure, cancellation, and unsupported empty results; it relaxes only after supported replacement research is published. Standalone Deep Research reports already in Outputs remain available and are disclosed in the confirmation. Display-name correction alone need not change identity when stable anchors match. An analytics snapshot with a different artist ID is a discrepancy to surface, not authority to overwrite the user's saved Spotify field.
