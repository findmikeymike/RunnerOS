...
- [x] **Builder specified September 15; implementation and live acceptance underway September 16.** [Spec 52 — Builder Agent](../creator-command-center/todo/52-builder-agent-spec.md) defines Agents/Workflows/Automations focuses, tool ownership, safe automation maintenance, evidence-led reuse, and a minimal development-profile transition. No production-version migration framework.
- [ ] **NOW — Finish Builder live acceptance.** Spec 52 A (identity/routing) and B (complete tools) are implemented; C (live proof) is in progress. Preserve saved work and connections; record implementation and live evidence separately.
...
- [ ] **NEXT — Define its location and handoff.** Likely a foundational Command worker. Manager and Assistant can route requests there without carrying the entire builder workflow in their own prompts. Preserve useful discovery/delegation in General.
