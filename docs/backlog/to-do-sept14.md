...
- [ ] **IMPLEMENTED — GRAVITY companion agent and skill; live acceptance next.** September 16: Michael supplied the cultural-gravity concept and approved a dedicated HQ agent centered on exceptional creative thinking, recent Intel and the agentic team's ability to research, build and operate. One reusable skill contains deeper creative, research and operating guidance. Manager retains career priorities; Builder creates capabilities. Recurring loops and spending remain separate decisions. See [implementation and verification](../audits/gravity-agent-2026-09-16.md). No live acceptance or release completion is implied.
...
- [ ] **NOW — Builder skill authoring and intel-led proposals (implemented; live acceptance pending).** Create/test/attach skills for new or existing workers; review relevant past/recent intel and suggest only strongly aligned, feasible capabilities. Reuse first; useful non-agentic intel and “build nothing” are valid outcomes. Auto-route completed reports with an initial rolling 60-day review, persistent dedupe and quiet no-op outcomes. Default to suggestions; ordinary scheduled/triggered Builder jobs use the existing Explore/Ask/Execute setting and task instructions to create something useful or skip. No separate approval/quota engine. No automatic connection or execution authority. Reuse Spec 36 reasoning without its broader engine. [Follow-up contract](../creator-command-center/todo/52-builder-agent-spec.md#11-requested-follow-up--skill-authoring-and-intel-led-capability-proposals). Not part of completed V1 acceptance.
  - **September 18 — Bounded live acceptance:** real Builder reviewed saved Industry intel and current catalogs, correctly recommended reuse/no new build, and performed no mutations in the review. Fixed default Builder browse omitting Industry and quiet-result detection rejecting a no-op status followed by an explanation. Source-reader check against saved HQ data and focused tests passed; updated app has not been restarted. Subsequent approved QA created, validated, revised and attached a custom skill while preserving the existing test worker's other fields. Seven direct Builder cases passed; the echo-only worker correctly retained its incompatible core behavior. A separate compatible QA worker then loaded the saved skill and passed two independently inspected delegated runs: recent date and missing-date/injection. Useful-proposal adoption on a real gap and loaded-build verification of the two fixes remain open. [Evidence and remaining checks](../audits/builder-intel-live-2026-09-18.md).
...
**September 18 — Bounded live handoff passed:** Artist Direction used existing HQ identity; Homebody Creative Direction saved a proposed brief; a fresh World Builder retrieved its actual idea and respected proposed status without execution. The proposed test brief remains saved for review. No HQ identity change or production approval. [Evidence](../audits/artist-creative-direction-upgrade-2026-09-16.md).
...
- [ ] **NEXT — Social publishing:** TryPost/Postiz and browser routes, correct account/media, receipt identity, ambiguous outcomes, and failed receipt storage. Verify uncertain publication is not blindly retried. Login verification alone is not publish acceptance.
  - **September 18 — Paused at Michael's request; return to Instagram/TikTok automation acceptance.** Manual walkthrough reached both final publish controls without posting. Follow-up fixes cover Instagram onboarding/stages, per-reel Facebook sharing, TikTok recovered uploads, background viewport sizing, and unfinished-draft preservation. Code is uncommitted; 137 focused tests, Electron typecheck and main build passed. Updated automation has not been live-tested or loaded by restart. Next: add an explicit stop-before-publish test mode that cannot click final Post/Share or create a publication receipt; save or explicitly resolve the open drafts before an authorized restart; exercise the actual scheduled execution path for both accounts and capture logs. Do not publish as part of this check. [Walkthrough, fixes and limits](../audits/social-prepublish-walkthrough-2026-09-17.md). This remains open, not release-certified.
- [ ] **NEXT — Campaign cleanup:** disposable campaign only; verify intended campaign data removal and preservation of global context and retained assets.
  - **Code review after `ce7c1e437`:** fixed a draft Output incorrectly removing preservation from an already-saved file. 62 focused tests and one disposable deletion journey passed. Durable-workflow activity during deletion remains the next bounded review; live acceptance is still open. [Evidence](../audits/campaign-cleanup-preservation-review.md).
