...
- [ ] **Finish Signals: Industry And Your World.** `NOW`. Host contracts, collection, durable coverage, tracked retry, opt-in workflows and the two-track reader/setup/audio (Slices 1-2) passed independent review and regression gates on `codex/signals-your-world`. Remaining: bounded content-worker retrieval and deliberate idea handoff (Slice 3), landing into main, and approved live acceptance. Source: [Signals: Industry And Your World](../creator-command-center/47-signals-your-world-spec.md).
...
- [ ] **Finish Catalog & Royalty Reconciliation.** `NEXT`. Implemented: `catalog-royalty-agent` persona, bundled `catalog-royalty-reconciliation` skill, default HQ worker slot, vault classification for registration documents. Remaining, in spec order: distributor CSV import with per-distributor guidance and ISRC capture (Slice 1), reconciliation engine with ISRC-exact and title-fuzzy matching and confidence levels (Slice 2), filing packets from Release Kit rights data (Slice 3), then the external-dependency slices — MLC Public Search API, Spotify completeness check, human-driven PRO capture, deadline tracking — each gated on the unresolved MLC and Spotify questions in the spec's Verification Status. Source: [Catalog And Royalty Reconciliation](../creator-command-center/31-catalog-royalty-reconciliation-spec.md).
