...
## Slice 14 — actionable admission blockers

Normal durable Start now identifies the first blocked step by its one-based position and provides static repair guidance for worker availability, task-mode selection, skill certification, launch receipt, tools, thinking, permissions, local sources, session context, or provider compatibility. Source and skill validation retain their internal error identifiers; typed categories survive SessionManager composition into Start. Unknown resolver failures use a generic verification message, never raw exception text, paths, prompts or credentials.

Authentication still precedes bundle inspection. All step bundles must resolve before admission, so a blocked later step starts no model work, creates no durable run and never falls back to legacy execution. Failure releases the pending-start guard, allowing a repaired configuration to retry. This changes diagnosis only: it enables no new skills, connectors, write tools or workflow definitions. The message reports one blocker at a time; it is not a full preflight report.


## Slice 15 — connected-source identity foundation (not enabled)

Added a host-only binding resolver for workspace-owned static bearer API sources. It captures the exact source configuration, readable guide, workspace directory identity, credential namespace and credential fingerprint, alongside a caller-supplied exact HTTPS URL list constrained to the configured origin/path. The returned binding contains no token, guide body or filesystem path. Configuration and credential reads are checked again across asynchronous lookup/hash boundaries; subsequent `assertCurrent` rejects removal, rotation, expiry, deactivation, source edits and directory replacement. Unreadable guides reject; missing and empty guides remain distinct.

This preparatory API is deliberately not imported by normal Start or registered as a model tool. It neither authorizes a caller nor certifies arbitrary GET endpoints as safe reads, validates DNS, performs network I/O, or changes journal approval identity. A saved binding must come from the host, never renderer input. First scope excludes global sources, OAuth/refresh, MCP, custom auth headers/schemes, query-bearing targets and connected writes. Bearer equality is a transport identity, not proof of the provider's human account identity or continued remote authorization.

Next integration work: certify a useful provider read route; add a bounded host-owned HTTPS transport with cancellation, public DNS pinning and no redirects; bind source credentials independently of model credentials in journal approvals and dispatch; then wire the tool through normal Start and prove result replay across process death. Existing public web reads and all current workflow behavior remain unchanged. Do not treat this foundation as connected-account workflow support.


## Slice 16 — host-owned authenticated GET transport (not enabled)

Added a bounded HTTPS JSON reader for the saved static-bearer source binding. It revalidates the connection/credential after DNS, then supplies the token only to the immediate host request callback. The transport accepts only a URL already in the saved list, pins a checked public IPv4 address into native HTTPS, verifies TLS/hostname, disables redirects and pooling, and sends no cookies or configurable headers. One 15-second deadline covers DNS, credential resolution and response streaming; cancellation/expiry fences late completions. Responses must be uncompressed JSON within 512 KiB; error bodies/headers are never returned, and direct token echoes (including JSON escapes) reject. The existing public-web IPv4 policy moved unchanged into a shared utility.

No new approvals are introduced. A host-supplied synchronous `isAuthorized` callback checks the existing policy/fence before lookup and immediately before request dispatch. It cannot ask for approval; false, exceptions and accidentally async callbacks deny dispatch. The future workflow bridge remains responsible for endpoint certification, existing permission/approval handling, source-specific journal identity, untrusted-content wrapping and saved-result replay. This code is not registered as a model tool or invoked by ordinary workflows. It performs no automatic retries, refresh, new source activation or writes. OAuth, MCP and global-source support remain outside the static-bearer pilot.


## Slice 17 — connected-read operation recovery (host-only)

Connected reads now use the existing encrypted operation journal through `createDurableConnectedReadAdapter` and `DurableEffectRunner`. Each intent saves its exact source binding and URL, source credential identity, adapter/output contract, attempt ceiling and unit reservation. The operation credential is independent of the model credential already by design; no new journal schema or parallel approval system was needed. A host must still certify the endpoint's read semantics and provide its existing noninteractive policy check.

Saved read outcomes reauthorize current source/account access before returning cached private data, without issuing another GET. Workspace-bound adapters cannot execute under another workspace's claim. A new journal dispatch guard rechecks the issued attempt, ownership, controls and deadline immediately before the transport dispatches after awaited DNS/credential work. A prevented dispatch remains retryable in the same slot after resume/repair; a real HTTP/error response is retained as an explicitly successful or failed tool observation (`ok`), marked untrusted. No prompt, automatic retry or broader permission is introduced.
