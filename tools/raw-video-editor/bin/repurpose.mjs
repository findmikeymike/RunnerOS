...
export async function executeRepurpose({ source, outDir, briefPath, render = false, sceneThreshold = 0.35 }) {
  ...
  if (!briefPath) {
    return {
      ...
      briefTemplatePath,
      guidance: 'Review the analysis, fill the variant brief, and rerun with --brief. Rendering remains off until the plan and rights are explicit.',
    };
  }
  ...
  manifest.status = 'rendered-for-review';
  manifest.completedAt = new Date().toISOString();
  writeJsonAtomic(manifestPath, manifest);
  return { ok: true, status: manifest.status, manifestPath, variants: manifest.variants };
}
