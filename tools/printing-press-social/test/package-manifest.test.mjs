...
test('root package ships and tests the Spotify CLI', () => {
  ...
  assert.deepEqual(spotifyFiles, [
    'spotify-cli/README.md',
    'spotify-cli/HARNESS.md',
    'spotify-cli/package.json',
    'spotify-cli/skills',
    'spotify-cli/src',
  ]);
  assert.match(rootPackage.scripts.test, /npm test --prefix spotify-cli/);
  assert.match(rootPackage.scripts.smoke, /npm run smoke --prefix spotify-cli/);
});

test('Spotify package test and smoke scripts stay runnable and dry-run only', () => {
