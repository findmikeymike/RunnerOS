  ...
  if (resolveBrowserEngine(flags, profile) === 'runner-cdp') {
    return {
      ...
      browserPlan: buildBrowserPlan({
        profile,
        sessionPath: sessionDir(profile),
        steps: ['open TikTok home', 'let the user log in manually', 'verify visible account matches profile'],
      }),
    };
  }
  ...
  if (resolveBrowserEngine(flags, profile) === 'runner-cdp') {
    return {
      ...
      browserPlan: buildBrowserPlan({
        profile,
        sessionPath: sessionDir(profile),
        steps: ['open TikTok home', 'verify visible account matches profile'],
      }),
    };
  }
...
TikTok MVP:
  ...
  social profile update tiktok --profile artist01 --handle @artist-main --account-url https://www.tiktok.com/@artist-main --json
